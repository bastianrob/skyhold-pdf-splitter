...
### Fixed
- **CLI Flag Collision**: Resolved a conflict between the global `--force` (`-f`) flag and the extraction `--from` (`-f`) flag.
- Extraction shorthands updated to `-F` (from) and `-T` (to) to ensure unique flag identification.
- Fixed documentation in `README.md` to reflect the updated shorthands.
